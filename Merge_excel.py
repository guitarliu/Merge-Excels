...
def merge_excels():
    ...
    if e_start.get() == "" or e_end.get() == "" or e_add_end.get() == "" or e_add_end.get() == "":
        ...
        for row2 in ws2_middle.rows:
            ...
            wb_final.save(os.getcwd() + "/" + "final.xlsx")
        messagebox.showinfo(title="合并完成", message="%s和%s合并完成，请到当前文件夹下查看！" % (var_file_1, var_file_2))
...
Label(root, text = "比如比较第1/2/3行，3/4/5列的内容", padx = 1, pady = 1).grid(row=6, column=1)
Label(root, text = "对对应行第7/../13连续列的内容相加").grid(row=7, column=1)
Button(root, text = "开始合并计算", command = merge_excels).grid(row=9, column = 1, padx = 1, pady = 5)
